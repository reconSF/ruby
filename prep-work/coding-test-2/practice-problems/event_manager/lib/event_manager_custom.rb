...
sendMail = check_input?("Would you like to generate thank you letters? (No to print info only)")
tallyReg = check_input?("Would you like check peak hourly and weekly registrations?")
...
contents.each { |row|
	...
	datetimes << row[:regdate] if tallyReg
...
registration_counts(datetimes) if tallyReg
